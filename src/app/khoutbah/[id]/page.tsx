...
export default function PageKhoutbah() {
  const { id } = useParams()
  const router = useRouter()
  const [khoutbah, setKhoutbah] = useState<Khoutbah | null>(null)
  const [toutes, setToutes] = useState<Khoutbah[]>([])
  ...
  useEffect(() => {
    async function charger() {
      const { data: kh } = await supabase.from('khoutbahs').select('*').eq('id', id).single()
      const { data: all } = await supabase.from('khoutbahs').select('*').order('serie').order('numero_serie').order('created_at', { ascending: false })
      if (kh) setKhoutbah(kh)
  ...
  const idx = toutes.findIndex(k => k.id === id)
  ...
  if (!khoutbah) return <main style={{ minHeight: '100vh', background: 'var(--fond-creme)' }}><Navbar /><div style={{ textAlign: 'center', padding: '100px 24px', color: '#aaa' }}>Introuvable</div></main>
    ...
    <main style={{ minHeight: '100vh', background: 'var(--fond-creme)' }}>
      ...
      <section style={{ background: 'var(--bleu)', padding: '40px 24px 36px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <Link href="/khoutbah" style={{ color: 'rgba(255,255,255,0.6)', fontSize: '13px', display: 'inline-block', marginBottom: '16px' }}>← Retour aux khoutbahs</Link>
          <h1 style={{ fontSize: '26px', fontWeight: 700, color: 'white', marginBottom: '6px', lineHeight: 1.3 }}>{khoutbah.titre}</h1>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '32px 24px' }}>
        <div style={{ background: 'white', border: '1px solid var(--bordure)', borderRadius: '16px', padding: '24px', marginBottom: '28px' }}>
          ...
          <h2 style={{ fontSize: '15px', fontWeight: 700, color: 'var(--texte)', marginBottom: '16px' }}>{khoutbah.titre}</h2>

          <audio ref={audioRef} src={khoutbah.url_audio} onPlay={() => setEnLecture(true)} onPause={() => setEnLecture(false)}
            ...
            onEnded={() => { setEnLecture(false); if (suivante) router.push('/khoutbah/' + suivante.id) }} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginTop: '14px' }}>
            <button onClick={() => precedente && router.push('/khoutbah/' + precedente.id)}
        ...
        <h3 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--texte)', marginBottom: '14px' }}>Toutes les khoutbahs</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {toutes.map(k => {
            const actif = k.id === id
            return (
              <Link key={k.id} href={'/khoutbah/' + k.id} style={{ background: actif ? '#e8f0f8' : 'white', border: '1px solid ' + (actif ? 'var(--bleu)' : 'var(--bordure)'), borderRadius: '10px', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '12px', textDecoration: 'none', transition: 'all 0.15s' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: '13px', fontWeight: actif ? 600 : 500, color: actif ? 'var(--bleu)' : 'var(--texte)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{k.titre}</p>
                  <p style={{ fontSize: '11px', color: '#999', marginTop: '2px' }}>{k.sheikh}</p>
                </div>
                {k.duree && <span style={{ fontSize: '11px', color: '#bbb', flexShrink: 0 }}>{k.duree}</span>}
              </Link>
            )
          })}
        </div>
