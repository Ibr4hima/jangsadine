...
export default function Conferences() {
  ...
  const idx = conferences.findIndex(c => c.id === actif?.id)
    ...
    <main style={{ minHeight: '100vh', background: 'var(--fond-creme)', display: 'flex', flexDirection: 'column' }}>
      ...
      <section style={{ background: 'var(--bleu)', padding: '48px 24px', textAlign: 'center' }}>
        ...
        <div style={{ maxWidth: '480px', margin: '0 auto', position: 'relative' }}>
          <input value={recherche} onChange={e => setRecherche(e.target.value)} placeholder="Rechercher par titre ou sheikh..." style={{ width: '100%', padding: '12px 20px 12px 44px', borderRadius: '50px', border: 'none', fontSize: '14px', fontFamily: 'inherit', outline: 'none', background: 'rgba(255,255,255,0.15)', color: 'white', boxSizing: 'border-box' }} />
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '32px 24px', flex: 1, width: '100%' }}>
        {actif && (
          <div style={{ background: 'white', border: '1px solid var(--bordure)', borderRadius: '16px', padding: '24px', marginBottom: '28px' }}>
            ...
            <h2 style={{ fontSize: '15px', fontWeight: 700, color: 'var(--texte)', marginBottom: '4px' }}>{actif.titre}</h2>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <p style={{ fontSize: '16px', color: '#aaa' }}>{recherche ? 'Aucune conference trouvee pour "' + recherche + '"' : 'Les conferences arrivent bientot'}</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {filtres.map(c => {
              const est = actif?.id === c.id
              return (
                <div key={c.id} onClick={() => setActif(c)}
