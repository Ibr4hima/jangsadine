...
function Squelettes({ n = 5 }: { n?: number }) {
  return (
...
// ─── Carte d'une conférence ───────────────────────────────────
function CarteConference({ c, actif, enLecture, onPress }: {
  c: Conference, actif: boolean, enLecture: boolean, onPress: () => void
}) {
  return (
    <div
      onClick={onPress}
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        />
        <p style={{ fontSize: 12, color: 'var(--texte-muted)', margin: 0 }}>{c.sheikh}</p>
      </div>
    </div>
  )
}
...
export default function Conferences() {
  ...
  useEffect(() => {
    supabase.from('conferences').select('*').order('created_at', { ascending: false })
      .then(({ data }) => { if (data) setConferences(data); setLoading(false) })
  }, [])
  ...
  const onPiste = (c: Conference) => {
    if (piste?.id === c.id) { toggleLecture(); return }
    jouer({ id: c.id, titre: c.titre, sheikh: c.sheikh, url: c.url_audio, duree: c.duree, href: '/conferences' })
  }
    ...
    <main style={{ minHeight: '100vh', background: 'var(--fond-creme)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 24px 80px', flex: 1, width: '100%' }}>
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {filtres.map((c, i) => (
              <div key={c.id} className="conf-item" style={{ animationDelay: `${Math.min(i, 8) * 45}ms` }}>
                <CarteConference
                  c={c}
                  actif={piste?.id === c.id}
                  enLecture={enLecture}
      ...
      <style>{`
        ...
        .conf-item { animation: confFadeUp 0.35s ease both; }
